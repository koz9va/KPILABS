...
public class Main {
    public static void main(String[] args) {
        double[] arrayOfUds = new double[sem3.lab1.Main.UdsLengthOfArray];
        double[] arrayOfUgs = new double[sem3.lab1.Main.UgsLengthOfArray];
        double[][] arrayOfCurrent = new double[sem3.lab1.Main.UgsLengthOfArray][sem3.lab1.Main.UdsLengthOfArray];
        try (BufferedReader console_reader = new BufferedReader(new InputStreamReader(System.in))) {
            ...
            try (BufferedReader fileReader = new BufferedReader(new FileReader("Current.txt"))) {
                while(fileReader.ready()){//пока есть строки
                    for (int i = 0; i < sem3.lab1.Main.UgsLengthOfArray; i++) {//проходим по строкам
                        String[] data = fileReader.readLine().split(" ");//читаем строку в массив через пробел
                        for (int j = 0; j < sem3.lab1.Main.UdsLengthOfArray; j++) {//проходим по столбцам
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename))) {
                for (double arrayOfUg : arrayOfUgs) {
                    double value = 0;
                    for (int i = 0; i < sem3.lab1.Main.UdsLengthOfArray * 2 - 1; i++) {
